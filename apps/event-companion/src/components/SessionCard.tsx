...
export function SessionCard({
  ...
  return (
    <article
      className={`rounded-sm border bg-paper-card p-4 shadow-soft transition motion-safe:hover:-translate-y-0.5 ${
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex flex-wrap items-center gap-2 text-xs font-medium uppercase tracking-wide text-ink-muted">
            {live ? (
              <span className="inline-flex items-center gap-1.5 rounded-sm bg-accent-soft px-2 py-0.5 text-accent-light">
                <span className="live-dot inline-block h-1.5 w-1.5 rounded-sm bg-accent" aria-hidden />
                On now
              </span>
            ) : null}
        ...
        <div className="flex shrink-0 flex-row flex-wrap items-center gap-2 sm:flex-col sm:items-stretch">
          <button
            ...
            className={`min-h-11 rounded-sm px-3 py-2 text-sm font-semibold transition ${
              saved
                ...
                : 'bg-ink/10 text-ink hover:bg-paper-mid'
            }`}
          >
            {saved ? 'Saved' : 'Save'}
          </button>
          <button
            type="button"
            onClick={onCalendar}
            className="min-h-11 rounded-sm border border-ink/15 bg-transparent px-3 py-2 text-sm font-medium text-ink-muted underline-offset-2 hover:border-ink/30 hover:text-ink hover:underline"
          >
            Add to calendar
          </button>
        </div>
